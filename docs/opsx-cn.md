...
默认情况下，OpenSpec 使用 `core` 工作流配置文件（`propose`、`explore`、`apply`、`archive`）。如果您想要扩展的工作流命令（`new`、`continue`、`ff`、`verify`、`sync`、`bulk-archive`、`onboard`），请使用 `openspec config profile` 进行配置，并使用 `openspec update` 进行应用。
...
| `context` | string | 项目上下文注入到所有产物指令中 |
| `rules` | object | 每个产物的规则，由产物 ID 键入 |
...
- 检查产物 ID 是否匹配你的 Schema (参见上面的列表)
- 运行 `openspec schemas --json` 查看每个 schemas 的产物 ID
...
| 命令 | 作用 |
|---------|--------------|
| `/opsx:propose` | 一步到位并生成规划文档并创建一个变更（默认快捷路径） |
| `/opsx:explore` | 思考想法、研究问题、明确需求 |
| `/opsx:new` | 开始一个新的变更脚手架（扩展工作流） |
| `/opsx:continue` | 基于已准备好的内容，创建下一个产物（扩展工作流） |
| `/opsx:ff` | Fast-forward —— 一次性创建所有规划的产物（扩展工作流） |
| `/opsx:apply` | 实施任务，根据需要更新产物 |
| `/opsx:sync` | 将增量规范(delta spec)同步到主规范(main spec)（扩展工作流，可选——如果需要，会提示归档） |
| `/opsx:archive` | 完成后归档 |
| `/opsx:bulk-archive` | 批量归档多个已完成的更改（扩展工作流） |
...
思考想法、研究问题，比较选项。不需要结构——只需一个有思想的合作伙伴。当见解具体化时，请过渡到 `/opsx:propose`（默认）、`/opsx:new` 或 `/opsx:ff`（扩展）。
...
根据依赖关系显示准备创建的内容，然后创建一个产物。反复使用以逐步建立您的更改。

```text
/opsx:ff add-dark-mode
```

一次性创建所有规划的产物。当您对正在构建的内容有清晰的了解时使用。
...
|---|---|---|
| **结构** | 一个大的提案文档 | 带有依赖关系的离散产物|
| **工作流** | 线性阶段：plan → implement → archive | 流畅的Action——随时做任何事 |
| **迭代** | 难以返回 | 在学习过程中更新产物|
| **自定义** | 固定结构 | 模式驱动（定义你自己的产物） |
...
本节中的示例使用了扩展命令集（如 `new`、`continue` 等）；默认的 `core` 用户可以将相同的流程映射为 `propose → apply → archive`。
...
- 在 `/opsx:apply` 期间，如果出现问题——修复产物，然后继续。
